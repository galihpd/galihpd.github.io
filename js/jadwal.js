document.addEventListener("DOMContentLoaded", function () {
        ...
        try {
            ...
            if(dataB.status && dataB.data.jadwal) {
                ...
                dataB.data.jadwal.forEach((hari, index) => {
                    ...
                    // --- FIX WARNA HIGHLIGHT ---
                    if(isToday) {
                        // Pakai style langsung biar pasti Putih Terang & Teks Hitam
                        row.style.backgroundColor = "white";
                        row.style.color = "#0f172a"; // Warna slate-900
                        row.classList.add("font-bold");
                    }

                    row.innerHTML = `
                        <td class="whitespace-nowrap" style="${isToday ? 'color: #0f172a !important;' : ''}">${namaHari[dateObj.getDay()]}, ${tglLoop} ${namaBulan[m-1]} ${y}</td>
                        <td class="text-center font-mono" style="${isToday ? 'color: #0f172a !important;' : ''}">${hari.subuh}</td>
                        <td class="text-center font-mono" style="${isToday ? 'color: #0f172a !important;' : ''}">${hari.dzuhur}</td>
                        <td class="text-center font-mono" style="${isToday ? 'color: #0f172a !important;' : ''}">${hari.ashar}</td>
                        <td class="text-center font-mono" style="${isToday ? 'color: #0f172a !important;' : ''}">${hari.maghrib}</td>
                        <td class="text-center font-mono" style="${isToday ? 'color: #0f172a !important;' : ''}">${hari.isya}</td>
                    `;
                    monthlyTableBody.appendChild(row);
                });
                const lokasiNama = citySearch.value || "KOTA JAKARTA";
                document.getElementById("printHeaderTitle").textContent = `Jadwal Sholat ${lokasiNama} - ${namaBulan[m-1]} ${y}`;
            }
        } catch (e) { console.error(e); }
    }
    ...
    function initApp() {
        ...
        selectedCityId = "1301";
        citySearch.value = ""; // Input kosong sesuai permintaan
    ...
    timezoneSelect.addEventListener("change", () => {
        ...
        monthlyTableBody.innerHTML = "<tr><td colspan='6' class='text-center opacity-50'>Silakan pilih kota</td></tr>";
    });
