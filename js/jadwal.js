document.addEventListener("DOMContentLoaded", function () {
    ...
    let jadwalSholat = {}; 
    let selectedCityId = "1301"; // Default Jakarta
    const namaBulan = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"];
    const namaHari = ["Minggu", "Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu"];

    // --- 1. DATABASE KOTA LENGKAP (500+) ---
    const dataKota = {
        "Asia/Jakarta": [
        ...
        try {
            ...
            if(dataB.status && dataB.data.jadwal) {
                ...
                dataB.data.jadwal.forEach((hari, index) => {
                    ...
                    if(isToday) row.classList.add("bg-secondary/20", "font-bold");

                    row.innerHTML = `
                        <td class="whitespace-nowrap">${namaHari[dateObj.getDay()]}, ${tglLoop} ${namaBulan[m-1]} ${y}</td>
                        <td class="text-center font-mono">${hari.subuh}</td>
                        <td class="text-center font-mono">${hari.dzuhur}</td>
                        <td class="text-center font-mono">${hari.ashar}</td>
                        <td class="text-center font-mono">${hari.maghrib}</td>
                        <td class="text-center font-mono">${hari.isya}</td>
                    `;
                    monthlyTableBody.appendChild(row);
                });
                document.getElementById("printHeaderTitle").textContent = `Jadwal Sholat ${citySearch.value} - ${namaBulan[m-1]} ${y}`;
            }
        } catch (e) { console.error(e); }
    }
    ...
    function updateClock() {
        ...
        fullDateDisplay.textContent = `${daySelect.value} ${namaBulan[monthSelect.value-1]} ${yearSelect.value}`;
        dayNameDisplay.textContent = namaHari[new Date(yearSelect.value, monthSelect.value-1, daySelect.value).getDay()];
        ...
        if (jadwalSholat.subuh) {
            ...
            ['subuh', 'dzuhur', 'ashar', 'maghrib', 'isya'].forEach(k => {
                const target = new Date(`${prefix}T${jadwalSholat[k]}:00`);
                const diff = target - now;
                const el = document.getElementById(`${k}Countdown`);
                if (diff <= 0) { el.textContent = "Adzan!"; el.classList.add("text-success"); }
                else {
    ...
    timezoneSelect.addEventListener("change", () => {
        const tz = timezoneSelect.value;
        citySearch.value = dataKota[tz][0].lokasi;
        selectedCityId = dataKota[tz][0].id;
        renderCities("");
        fetchAllData();
    });
