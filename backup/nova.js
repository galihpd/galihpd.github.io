...
async function fetchISSPosition() {
    ...
    try {
        ...
        try {
            ...
            if(fallbackData.latitude) {
                const lat = fallbackData.latitude;
                const lon = fallbackData.longitude;
                const pos = latLongToVector3(lat, lon, 1.05);
...
function showLaunchDetail(item, lock = false) {
    ...
    let lat = null, lon = null;
    if(item.pad && item.pad.latitude !== null) { lat = item.pad.latitude; lon = item.pad.longitude; }
    if(item.isSpaceport || item.isUser) { lat = item.lat; lon = item.lon; }

    if (lock && lat !== null && lon !== null) flyToLatLon(lat, lon);
...
function setupControls() {
    ...
    document.getElementById('reset-view').addEventListener('click', () => flyToLatLon(-0.7893, 113.9213));
}
